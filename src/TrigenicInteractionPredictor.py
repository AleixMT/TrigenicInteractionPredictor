...
class Model:
    ...
    def get_input(self, argfilename, selectedinteractiontype="trigenic", cutoffvalue=-0.08, discard=0,
                  interactions='ALL'):
        try:
            gid = 0
        ...
        try:
            ...
            with codecs.open(testfile, encoding='utf-8', mode='r') as fileref:
                ...
                for line in fileref.readlines():
                    ...
                    try:
                        self.test_links[str_gene_triplet][
                            r] += 1  # link between g1, g2 and g3 with rating r it's been seen +1 times
                    except KeyError:  # if link between n1 and n2 with rating r is the first time seen then
    ...
    def fold(self, fraction=0.2):
        ...
        for num_fold in range(num_folds):
            ...
            for triplet in trains[num_fold]:
    ...
    def compute_likelihood(self, selected_set='train'):
        if selected_set == 'train':
            links_set = self.links.items()
        else:
            links_set = self.test_links.items()
        log_l = 0.
        for triplet, rating_vector in links_set:  # e X ra iteration
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hi:n:s:f:b:o:t:e:k:",
                                   ["help", "num_iterations=", "num_samples=", "sample_ini=", "fcheck=", "bcheck=", "out=", "train=", "test=", "k="])
    ...
    msg += "**************************\n\nDoing " + str(numSamples) + " samples of " + str(iterations) + " num_iterations."
    msg += "\nTrain-file is " + str(train) + "\n Test-file is " + str(test) + "\n Output directory is " + str(outfilepath)
    msg += "\nK value (number of groups) is " + str(argk) + "."
    msg += "\nLikelihood will be computed every " + str(frequencyCheck) + " num_iterations after iteration number " + str(beginCheck)
    print(msg)
